...
- **Not already covered — hard ban, no exceptions:** if a specific tool, app, or named technique appears in ANY existing entry in EXISTING COVERAGE, it is completely banned as the primary focus of a new article. No "different angle," no "different audience," no exceptions. If a tool appears in even one existing entry, zero new articles centered on it. The only way a covered tool appears in a new article is as a passing mention inside an article primarily about something else. (This is also enforced in code downstream — a violating entry is dropped automatically, so proposing one just wastes a slot.)
